...
function getDateRangeFromChileKey(dateKey) {
...
function parseHoraProgramada(value) {
    if (!value)
        return null;
...
function parseJsonCoordinates(value) {
    ...
    const readFromRecord = (item) => {
        const latitud = toNumber(item.latitud) ??
            toNumber(item.latitude) ??
            toNumber(item.lat) ??
            toNumber(item.y);
        const longitud = toNumber(item.longitud) ??
        ...
        if (latitud === null || longitud === null)
            return null;
        const direccion = typeof item.direccion === "string"
            ? item.direccion
    ...
    if (isRecord(value)) {
        ...
        for (const item of Object.values(value)) {
            ...
            if (nested.latitud !== null && nested.longitud !== null)
                return nested;
        }
    }
...
function mapAgendaAsignada(visita, empresa) {
    ...
    const inconsistenciaEstado = visita.visita?.status === client_1.EstadoVisita.COMPLETADA &&
        visita.estado !== client_1.EstadoAgenda.COMPLETADA
...
const obtenerMisVisitasAsignadasHoy = async (req, res) => {
    try {
        ...
        const data = visitas.map((visita) => mapAgendaAsignada(visita, visita.empresaId ? empresasById.get(visita.empresaId) : undefined));
...
exports.obtenerMisVisitasAsignadasHoy = obtenerMisVisitasAsignadasHoy;
const obtenerVisitaDeAgenda = async (req, res) => {
    try {
        const tecnicoId = req.user?.id;
...
exports.obtenerVisitaDeAgenda = obtenerVisitaDeAgenda;
const iniciarRutaAgendaVisita = async (req, res) => {
    try {
        const tecnicoId = req.user?.id;
        const agendaId = Number(req.params.id);
        if (!tecnicoId)
        ...
        if (!visita)
            return res.status(404).json({ error: "Visita asignada no encontrada" });
        if (visita.visita?.status === client_1.EstadoVisita.COMPLETADA) {
        ...
        if (visita.estado === client_1.EstadoAgenda.COMPLETADA || visita.estado === client_1.EstadoAgenda.CANCELADA) {
...
exports.iniciarRutaAgendaVisita = iniciarRutaAgendaVisita;
const iniciarVisitaAgendaVisita = async (req, res) => {
    try {
        const tecnicoId = req.user?.id;
        const agendaId = Number(req.params.id);
        if (!tecnicoId)
        ...
        if (visita.estado === client_1.EstadoAgenda.PROGRAMADA || visita.estado === client_1.EstadoAgenda.NOTIFICADA) {
...
exports.iniciarVisitaAgendaVisita = iniciarVisitaAgendaVisita;
const finalizarAgendaVisita = async (req, res) => {
    try {
        const tecnicoId = req.user?.id;
        const agendaId = Number(req.params.id);
        if (!tecnicoId)
        ...
        if (!visita)
            return res.status(404).json({ error: "Visita asignada no encontrada" });
        if (visita.estado === client_1.EstadoAgenda.COMPLETADA) {
            const empresa = await cargarEmpresaAgenda(visita.empresaId);
            return res.json({
                visita: mapAgendaAsignada(visita, empresa ?? undefined),
            });
        }
        if (visita.estado === client_1.EstadoAgenda.CANCELADA) {
        ...
        if (formulario.status === client_1.EstadoVisita.PENDIENTE) {
            return res.status(409).json({
                error: "El formulario de visita debe estar completado antes de cerrar la agenda.",
            });
        }
        if (formulario.status !== client_1.EstadoVisita.COMPLETADA) {
            return res.status(409).json({
                error: "El formulario de visita debe estar completado antes de cerrar la agenda.",
            });
        }
        if (visita.estado !== client_1.EstadoAgenda.INICIADA &&
            visita.estado !== client_1.EstadoAgenda.PROGRAMADA &&
            visita.estado !== client_1.EstadoAgenda.NOTIFICADA &&
            visita.estado !== client_1.EstadoAgenda.EN_RUTA) {
            return res.status(409).json({ error: "La agenda no se puede finalizar desde su estado actual." });
        }
        if (visita.estado !== client_1.EstadoAgenda.INICIADA) {
            console.warn(`[AGENDA] Reparando agenda #${agendaId}: formulario #${formulario.id_visita} COMPLETADA con agenda ${visita.estado}.`);
        }
        const actualizada = await prisma_js_1.prisma.agendaVisita.update({
            where: { id: agendaId },
            data: {
...
exports.finalizarAgendaVisita = finalizarAgendaVisita;
const registrarUbicacionTecnico = async (req, res) => {
    try {
        const tecnicoId = req.user?.id;
        if (!tecnicoId)
            return res.status(401).json({ error: "No autenticado" });
        const { agendaId, latitud, longitud, precision, velocidad, estadoTracking, } = req.body ?? {};
...
exports.registrarUbicacionTecnico = registrarUbicacionTecnico;
...
function coordenadasVisita(latitud, longitud) {
...
const crearVisita = async (req, res) => {
    try {
        ...
        if (parsedAgendaId !== null) {
            ...
            if (agenda.estado !== client_1.EstadoAgenda.INICIADA) {
            ...
            try {
                nuevaVisita = await crearVisitaInicial({
                    ...
                    inicio,
                    agendaId: agenda.id,
                    origen: client_1.OrigenVisita.AGENDA,
                });
            }
            catch (error) {
                if (error?.code === "P2002") {
        ...
        const nuevaVisita = await crearVisitaInicial({
            ...
            inicio,
            agendaId: null,
            origen: client_1.OrigenVisita.MANUAL,
        });
